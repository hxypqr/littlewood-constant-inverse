...
\usepackage{lmodern}
\usepackage[a4paper,textwidth=6.15in,textheight=9.15in,centering]{geometry}
...
\setlist[enumerate]{leftmargin=*,label=\textup{(\roman*)},itemsep=3pt,topsep=5pt}
...
The argument refines the McGehee--Pigno--Smith construction analysed by
Bloom and Green: outer multipliers use an exact pointwise modulus budget,
and a whole-product estimate couples their error to the loss of squared
modulus. All even moments are controlled by interval moments.
We also prove a structural inverse theorem for an explicit class of integer
sets generated from dense separated blocks and long arithmetic block
inflations by cyclic Sidon-fibre decompositions of arbitrary depth.
If $\La(A)\le K\log|A|$, then, for every $\eps>0$, all but $\eps|A|$ points
are covered by boundedly many disjoint, linear-sized subsets of bounded
doubling. The constants are independent of the decomposition depth,
branching, moduli, and diameter. The proof combines a constant-one Sidon
coefficient inequality with hereditary energy estimates obtained from
bounded majority amplification and integer boundaries. The conclusion
survives high-order Freiman modelling. The decomposition hypothesis is
substantive; no unrestricted linear-sized inverse theorem or sharp
$4/\pi^2$ lower bound is asserted.
\end{abstract}
\maketitle
\tableofcontents
\newpage
...
\section*{Computational reproducibility and assistance}
...
for the proofs. Generative-AI tools were used substantially in developing
and testing arguments, organizing the manuscript, and preparing the
exposition and code. Such assistance does not constitute an independent
verification of the mathematics. Responsibility for the submitted
